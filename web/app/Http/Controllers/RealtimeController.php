...
class RealtimeController extends Controller
{
    public function stream(Request $request)
    {
        ...
        $newSensor = SensorLog::where('id_sensor', '>', $lastSensorId)
        ...
        $newGps = GpsLog::where('id_gps', '>', $lastGpsId)
            ->orderBy('id_gps', 'asc')
            ->first();
        if ($newGps) {
        ...
        $newSos = SosEvent::where('id_sos', '>', $lastSosId)
            ->orderBy('id_sos', 'asc')
            ->first();
        if ($newSos) {
        ...
        // Fetch latest state values to sync
        $device = Device::first();
        if ($device) {
